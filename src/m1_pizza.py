...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    expected = [rg.Point(180.0, 50.0),
                rg.Point(173.1, 59.5),
                rg.Point(161.9, 55.9),
                rg.Point(161.9, 44.1),
                rg.Point(173.1, 40.5)]
    circle = rg.Circle(rg.Point(170, 50), 10)
...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    # DONR: 3. Implement this TEST function.
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    title = 'PIZZA test 1: 10 slices,thin(thickness=1) black lines.'
    window = rg.RoseWindow(500, 500, title)
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON tests 3:  80 segments with thin green lines'
    window = rg.RoseWindow(500, 500, title)

    circle = rg.Circle(rg.Point(250,250), 200)
    circle.outline_thickness = 1
    polygon(window, circle, 80, 'green', 3)
    window.continue_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(len(points) - 1):
        ...
        lines2.color = lines1.color
        lines2.thickness = lines1.thickness
